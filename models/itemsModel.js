...
const read_all_items = async () => {
	const [result] = await db.query(`SELECT items.name as itemName, items.registered_date, items.purchase_cost, items.size, items.barcode, accounts.name as accountName, accounts.margin_ratio FROM items LEFT JOIN accounts ON items.account_id = accounts.id`);
	if(result == undefined){
		return null;
	}else{
		return result;
